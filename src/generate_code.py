...
def element_code(e, indent = 1):
	...
	if e['type'] == 'tag':
		...
		e_code.append(addProps(e['varname'], e['attrs'], indent))
		e_code.append('\n')
		for c in e['childs']: e_code += element_code(c, indent)
...
def appendAll(e, indent = 1):
	...
	append_code = []
	for c in e['childs']:
		append_code += appendAll(c, indent)
		append_code.append(appendChild(e, c, indent))
...
def appendChild(parent, child, indent = 1):
	...
	if arg == "''" or arg == "' '": return ""	#Avoid appendChild(document.createTextNode(' '))
...
def generate_function(args, element_tree, repo, src_code, with_header = True, indent = 0):
	lines = []
	...
	if with_header:
		...
		lines += ['{}//{}\n'.format('\t'*indent, l) for l in src_code.split('\n') if l != ""]

	lines += ['{}function {}({}) {}\n'.format('\t'*indent, args.function_name, ', '.join(p['varname'] for p in args.param), '{')]
	lines += [''.join(element_code(e, indent + 1)) for e in element_tree]
	lines += [''.join(appendAll(e, indent + 1)) for e in element_tree]
	lines += [get_return(element_tree, indent + 1)]
	lines += ['{}{}\n'.format('\t'*indent, '}')]
	return lines
